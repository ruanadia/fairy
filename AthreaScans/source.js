...
var _Sources = (() => {
  ...
  // src/AthreaScans/AthreaScans.ts
  var AthreaScans_exports = {};
  __export(AthreaScans_exports, {
    AthreaScans: () => AthreaScans,
    AthreaScansInfo: () => AthreaScansInfo
  });
  ...
  // src/AthreaScans/AthreaScans.ts
  var DOMAIN = "https://athreascans.com";
  var AthreaScansInfo = {
    version: "1.0.0",
    name: "AthreaScans",
    icon: "icon.png",
  ...
  var AthreaScans = class extends import_types2.Source {
    constructor() {
      super(...arguments);
    ...
    async getSearchResults(query, metadata) {
      ...
      const $2 = load(response.data ?? "");
      const tiles = [];
      for (const item of $2(".listupd .bsx").toArray()) {
        const link = $2(item).find("a");
        const id = link.attr("href")?.split("/manga/")[1]?.replace(/\/$/, "");
        const title = link.attr("title");
        const image = $2(item).find("img").attr("src") ?? "";
        if (id && title) {
          tiles.push(App.createPartialSourceManga({ mangaId: id, image, title, subtitle: void 0 }));
        }
      }
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const $2 = load(response.data ?? "");
      const featuredItems = [];
      for (const item of $2(".slidernom3 .swiper-slide").toArray()) {
        const link = $2(item).find("a");
        const id = link.attr("href")?.split("/manga/")[1]?.replace(/\/$/, "");
        const title = $2(item).find(".name").text().trim();
        const image = $2(item).find("img").attr("src") ?? "";
        if (id && title) featuredItems.push(App.createPartialSourceManga({ mangaId: id, title, image, subtitle: void 0 }));
      }
      ...
      for (const item of $2(".postbody .listupd .bsx").toArray()) {
        ...
        const image = $2(item).find("img").attr("src") ?? "";
        if (id && title) latestItems.push(App.createPartialSourceManga({ mangaId: id, title, image, subtitle: void 0 }));
      }
      sectionLatest.items = latestItems;
      sectionCallback(sectionLatest);
      const newItems = [];
      for (const item of $2("#sidebar .serieslist ul li").toArray()) {
        const link = $2(item).find("a.series");
        const id = link.attr("href")?.split("/manga/")[1]?.replace(/\/$/, "");
        const title = link.text().trim() || link.find("img").attr("title") || "";
        const image = $2(item).find("img").attr("src") ?? "";
        if (id && title) newItems.push(App.createPartialSourceManga({ mangaId: id, title, image, subtitle: void 0 }));
      }
      sectionNew.items = newItems;
      sectionCallback(sectionNew);
    }
  };
  return __toCommonJS(AthreaScans_exports);
})();
