...
var _Sources = (() => {
  ...
  var VioletScans_exports = {};
  __export(VioletScans_exports, {
    VioletScans: () => VioletScans,
    VioletScansInfo: () => VioletScansInfo
  });
  ...
  function filterBySelector(selector, elements, options) {
    ...
    if (selector.some(isTraversal)) {
      const root3 = (_a4 = options.root) !== null && _a4 !== void 0 ? _a4 : getDocumentRoot(elements[0]);
  ...
  function isNumber4(c) {
    return c >= CharCodes4.Zero && c <= CharCodes4.Nine;
  }
  function isASCIIAlpha(c) {
    return c >= CharCodes4.LowerA && c <= CharCodes4.LowerZ || c >= CharCodes4.UpperA && c <= CharCodes4.UpperZ;
  }
  function isHexDigit(c) {
    return c >= CharCodes4.UpperA && c <= CharCodes4.UpperF || c >= CharCodes4.LowerA && c <= CharCodes4.LowerF;
  }
  var QuoteType;
  (function(QuoteType2) {
  ...
  var Tokenizer2 = class {
    ...
    handleInAttributeValue(c, quote) {
      if (c === quote || !this.decodeEntities && this.fastForwardTo(quote)) {
        ...
        this.sectionStart = -1;
        this.cbs.onattribend(quote === CharCodes4.DoubleQuote ? QuoteType.Double : QuoteType.Single, this.index);
        this.state = State2.BeforeAttributeName;
      } else if (this.decodeEntities && c === CharCodes4.Amp) {
        this.baseState = this.state;
        this.state = State2.BeforeEntity;
    ...
    stateBeforeEntity(c) {
      this.entityExcess = 1;
      this.entityResult = 0;
      if (c === CharCodes4.Number) {
        this.state = State2.BeforeNumericEntity;
      } else if (c === CharCodes4.Amp) {
      } else {
        this.trieIndex = 0;
    ...
    stateInNamedEntity(c) {
      this.entityExcess += 1;
      this.trieIndex = determineBranch3(this.entityTrie, this.trieCurrent, this.trieIndex + 1, c);
      if (this.trieIndex < 0) {
      ...
      const masked = this.trieCurrent & BinTrieFlags3.VALUE_LENGTH;
      if (masked) {
        const valueLength = (masked >> 14) - 1;
        if (!this.allowLegacyEntity() && c !== CharCodes4.Semi) {
    ...
    emitNamedEntity() {
      ...
      const valueLength = (this.entityTrie[this.entityResult] & BinTrieFlags3.VALUE_LENGTH) >> 14;
      switch (valueLength) {
        case 1: {
          this.emitCodePoint(this.entityTrie[this.entityResult] & ~BinTrieFlags3.VALUE_LENGTH);
          break;
        }
        case 2: {
    ...
    emitNumericEntity(strict) {
      ...
      if (numberStart !== this.index) {
        ...
        this.emitCodePoint(replaceCodePoint3(this.entityResult));
      }
      this.state = this.baseState;
    }
    stateInNumericEntity(c) {
      if (c === CharCodes4.Semi) {
        this.emitNumericEntity(true);
      } else if (isNumber4(c)) {
        this.entityResult = this.entityResult * 10 + (c - CharCodes4.Zero);
        this.entityExcess++;
      } else {
    ...
    stateInHexEntity(c) {
      if (c === CharCodes4.Semi) {
        ...
        this.entityResult = this.entityResult * 16 + (c - CharCodes4.Zero);
        this.entityExcess++;
      } else if (isHexDigit(c)) {
        this.entityResult = this.entityResult * 16 + ((c | 32) - CharCodes4.LowerA + 10);
        this.entityExcess++;
      } else {
        if (this.allowLegacyEntity()) {
          this.emitNumericEntity(false);
  ...
  var VioletScans = class extends import_types2.Source {
    constructor() {
      super(...arguments);
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const $2 = load(response.data ?? "");
      const featuredItems = [];
      for (const item of $2(".slidernew .swiper-slide").toArray()) {
        const link = $2(item).find("a");
        ...
        if (id && title) {
          featuredItems.push(App.createPartialSourceManga({ mangaId: id, title, image, subtitle: void 0 }));
        }
      }
      sectionFeatured.items = featuredItems;
      sectionCallback(sectionFeatured);
      const popularItems = [];
      for (const item of $2(".hotslid .pop-list .bsx").toArray()) {
        const link = $2(item).find("a");
        const id = link.attr("href")?.split("/comics/")[1]?.replace(/\/$/, "");
      ...
      sectionCallback(sectionPopular);
      const newItems = [];
      for (const item of $2(".postbody .pop-list .bsx").toArray()) {
        const link = $2(item).find("a");
        const id = link.attr("href")?.split("/comics/")[1]?.replace(/\/$/, "");
        const title = link.attr("title");
        const image = $2(item).find("img").attr("src") ?? "";
        if (id && title) {
          newItems.push(App.createPartialSourceManga({ mangaId: id, title, image, subtitle: void 0 }));
        }
      }
      sectionNew.items = newItems;
      sectionCallback(sectionNew);
      const latestItems = [];
      for (const item of $2(".latest-updates .bsx").toArray()) {
        const link = $2(item).find("a");
        const id = link.attr("href")?.split("/comics/")[1]?.replace(/\/$/, "");
        const title = link.attr("title");
        const image = $2(item).find("img").attr("src") ?? "";
  ...
  return __toCommonJS(VioletScans_exports);
})();
